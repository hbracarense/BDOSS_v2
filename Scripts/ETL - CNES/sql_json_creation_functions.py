import os
import pandas as pd
from general_functions import check_file, check_lower, check_type_sql, check_type_dtypes_json, check_type_read_json, create_dir
...
def create_json(path_file, file_name, df, col_name, type_name, date_cols, dtypes: bool):
    check_file(os.path.join(path_file, file_name))
    with open(os.path.join(path_file, file_name), 'w', encoding = 'utf-8') as json:
        ...
        for i in range(len(df)):
            if dtypes:
                ...
                column = df.loc[i, col_name]
                ctype = check_type_read_json(df.loc[i, type_name], df.loc[i, col_name], date_cols)
                last_string = '\n}'
...
def create_sql_json_files(path_abs, sql_folder, json_folder, tables_folder, path_file_dic, col_name, type_name, comment_col, date_cols):
    ...
    for i in range(1,len(sheets)):
        try:
            df =  pd.read_excel(path_file_dic, sheet_name = sheets[i])
        except Exception as e:
            return print(e)
        main_dir = sheets[i].split('-')[0]
        table = check_lower(sheets[i].split('-')[1])
        print('Sheet name {sheet} imported as dataframe with success.'.format(sheet = sheets[i]))
        create_dir(os.path.join(path_abs, sql_folder, tables_folder), main_dir)
        create_dir(os.path.join(path_abs, json_folder), main_dir)
        path_sql = os.path.join(path_abs, sql_folder, tables_folder, main_dir)
        path_json = os.path.join(path_abs, json_folder, main_dir)
        table_name = 'tb_{db_name}_{tb_name}'.format(db_name = check_lower(main_dir), tb_name = table)
        ...
        json_read_file = table_name + '_read.json'
        ...
        create_json(path_json, json_dtypes_file, df, col_name, type_name, date_cols, dtypes=True)
        print('JSON dtypes file {file} created with success.'.format(file = json_dtypes_file))
        create_json(path_json, json_read_file, df, col_name, type_name, date_cols, dtypes=False)
        print('JSON read file {file} created with success.'.format(file = json_read_file))
...
def create_schema_sql(path_abs, sql_folder, schema_folder, path_file_dic):
    ...
    with pd.ExcelFile(path_file_dic) as dictionary:
        sheets = dictionary.sheet_names
    print('Sheet names imported with success.')
    sheets = list(set([x.split('-')[0] for x in sheets[1:]]))
