...
def create_sql(path_file, file_name, df, schema, table, col_name, type_name, comment_col):
    ...
    with open(os.path.join(path_file, file_name), 'w', encoding = 'utf-8') as sql:
        ...
        for i in range(len(df)):
            sql.write('\n\t{column_name} {variable_type} NULL'.format(column_name = check_lower(df.loc[i, col_name]), variable_type = check_type_sql(df.loc[i, type_name])))
            if i < len(df) - 1:
                sql.write(',')
...
def create_json(path_file, file_name, df, col_name, type_name):
    ...
    with open(os.path.join(path_file, file_name), 'w', encoding = 'utf-8') as json:
        ...
        for i in range(len(df)):
            ...
            ctype = check_type_dtypes_json(df.loc[i, type_name])
