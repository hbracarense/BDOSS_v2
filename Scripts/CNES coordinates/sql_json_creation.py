...
def create_sql(path_file, file_name, df, schema, table, col_name, type_name, comment_col):
    ...
    with open(os.path.join(path_file, file_name), 'w', encoding = 'utf-8') as sql:
        ...
        for i in range(len(df)):
            sql.write('\n\t{column_name} {variable_type} NULL'.format(column_name = check_lower(df.loc[i, col_name]), variable_type = check_type_sql(df.loc[i, type_name], df.loc[i, col_name])))
            if i < len(df) - 1:
                sql.write(',')
...
def create_json(path_file, file_name, df, col_name, type_name):
    check_file(os.path.join(path_file, file_name))
    with open(os.path.join(path_file, file_name), 'w', encoding = 'utf-8') as json:
        json.write('{')
        for i in range(len(df)):
            column = check_lower(df.loc[i, col_name])
            ctype = check_type_dtypes_json(df.loc[i, type_name], df.loc[i, col_name])
...
def create_sql_json_files(path_abs, sql_folder, json_folder, tables_folder, path_file_dic, col_name, type_name, comment_col, table, schema):
    ...
    for sheet in sheets:
        ...
        create_sql(path_sql, sql_file, df, schema, table_name, col_name, type_name, comment_col)
        print('SQL file {file} created with success.'.format(file = sql_file))
        create_json(path_json, json_dtypes_file, df, col_name, type_name)
