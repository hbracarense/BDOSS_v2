...
def read_and_write_df_into_sql(path_abs, path_resources, path_json, json_file_name, path_file, file_data_name, schema, table, engine):
    try:
        df = pd.read_csv(os.path.join(path_file, file_data_name), sep = ';', encoding = 'latin1')
        df.rename(columns={'CODIGO':'codigo', 'TITULO':'titulo'}, inplace = True)
    ...
    try:
        ...
        print('CBO data inserted in table with success!')
    except Exception as e:
        print(e)
        return None    